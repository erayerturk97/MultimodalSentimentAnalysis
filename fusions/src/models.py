...
class TextClassifier(nn.Module):
    def __init__(self, config):
        ...
        if self.config.text_encoder == 'glove':
            ...
            self.rnn1 = rnn(self.input_size, self.hidden_size, bidirectional=True)
            self.rnn2 = rnn(2*self.hidden_size, self.hidden_size, bidirectional=True)
    ...
    def forward(self, sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask):
        ...
        if self.config.text_encoder == 'glove':
            ...
            packed_sequence = pack_padded_sequence(sentences, lengths.cpu())
...
class VisualClassifier(nn.Module):
    ...
    def forward(self, sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask):
        batch_size = lengths.size(0)
        packed_sequence = pack_padded_sequence(video, lengths.cpu())
...
class AcousticClassifier(nn.Module):
    ...
    def forward(self, sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask):
        batch_size = lengths.size(0)
        packed_sequence = pack_padded_sequence(acoustic, lengths.cpu())
...
class EarlyFusion(nn.Module):
    ...
    def forward(self, sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask):
        _, text_features = self.text_classifier(sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask)
        _, visual_features = self.visual_classifier(sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask)
        _, acoustic_features = self.acoustic_classifier(sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask)
...
class LateFusion(nn.Module):
    ...
    def forward(self, sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask):
        text_outs, _ = self.text_classifier(sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask)
        visual_outs, _  = self.visual_classifier(sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask)
        acoustic_outs, _ = self.acoustic_classifier(sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask)
